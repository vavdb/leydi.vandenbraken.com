<template>
  <section class="resume-section p-3 p-lg-5 d-flex align-items-center" id="educatie">
    <div class="w-100">
      <h2 class="mb-5">Educatie</h2>

      <div class="resume-item d-flex flex-column flex-md-row justify-content-between mb-5">
        <div class="resume-content">
          <h3 class="mb-0">Bestuurs- &amp; Organisatiewetenschappen</h3>
          <div class="subheading mb-3">Utrecht University</div>
          <div>Research Master met Honourstraject</div>
          <p>
            Timeseries analyse van alle KNVB ledengegevens gekoppeld met CBS
            data, weergave in Qlikview
          </p>
          <p>Honours: stakeholdermanagement; jongeren in RvTs</p>
        </div>
        <div class="resume-date text-md-right">
          <span class="text-primary">sep 2012 - nov 2014</span>
        </div>
      </div>

      <div class="resume-item d-flex flex-column flex-md-row justify-content-between">
        <div class="resume-content">
          <h3 class="mb-0">Arbeid, Zorg &amp; Welzijn: beleid &amp; interventie</h3>
          <div class="subheading mb-3">Utrecht University</div>
          <p>Onderzoek naar Social Impact Bonds, interviews</p>
          <p>Semester UC Berkeley, CA, USA in Social Welfare</p>
        </div>
        <div class="resume-date text-md-right">
          <span class="text-primary">sep 2013 - jul 2014</span>
        </div>
      </div>

      <div class="resume-item d-flex flex-column flex-md-row justify-content-between">
        <div class="resume-content">
          <h3 class="mb-0">Culturele Antropologie</h3>
          <div class="subheading mb-3">Utrecht University</div>
          <p>
            Onderzoek naar maatschappelijke participatie door sport (o.a. WMO),
            gemeente Utrecht, interviews
          </p>
          <p>
            Minors: Bestuurskunde, interculturele communicatie, ondernemerschap,
            spaanse taal &amp; cultuur
          </p>
          <p>Stages: Port4Growth, SOON / gemeente Utrecht</p>
        </div>
        <div class="resume-date text-md-right">
          <span class="text-primary">sep 2013 - jul 2014</span>
        </div>
      </div>
    </div>
  </section>
</template>
